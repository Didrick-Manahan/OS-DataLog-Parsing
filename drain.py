...
class Drain:
    ...
    def addSeqToPrefixTree(self, rn, logClust):
        ...
        if seqLen not in rn.childD:
            firstLayerNode = Node(depth=1, digitOrtoken=seqLen)
            rn.childD[seqLen] = firstLayerNode
        else:
            firstLayerNode = rn.childD[seqLen]

        parentn = firstLayerNode
    ...
    def main(self):
        ...
        print('duration: {:.5f}'.format(t2-t1))
        gc.collect()

        return t2-t1


# # rough LinuxLog params
# path = './data/'
# logName = 'LinuxLog.log'
# removeCol = [0,1,2,3,4]
# st = 0.5
# depth = 4
# rex = ['(/|)([0-9]+\.){3}[0-9]+(:[0-9]+|)(:|)']

# # rough OpenStack params
# path = './data/'
# logName = 'OpenStack.log'
# removeCol = [0,1,2,3,4,5]
# st = 0.5
# depth = 4
# rex = ['req-[a-f0-9]{8}(-[a-f0-9]{4}){3}-[a-f0-9]{12}', '[a-f0-9]{32}']

# rough HDFS params
path = './data/'
logName = 'ProcessedSampledWindowsLog.log'
savePath = './results/drain/'
removeCol = [0]
st = 0.5
depth = 4
...
parserPara = DrainParams(path=path, st=st, removeCol=removeCol, rex=rex, depth=depth, logName=logName, savePath=savePath)   
drain = Drain(parserPara)
drain.main()
